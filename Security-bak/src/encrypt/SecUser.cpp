...
SecUser::SecUser(Name &remotePrefix,Name &remoteSuffix, Name &selfPrefix, Name &selfSuffix)
{
    Name keyName(selfPrefix);
    keyName.append(Encryptor::getNAME_COMPONENT_SAMPLE()).append(selfSuffix).append("HMACwithSha256");

    //keyName("/Prefix/SAMPLE/Content/HMACwithSha256");
    ...
    secUserMainFace_.setCommandSigningInfo(keyChain_, certificateName);

    Name groupName(remotePrefix);
    groupName.append(Encryptor::getNAME_COMPONENT_READ());

    Name secUserName(remotePrefix);
    secUserName.append(Encryptor::getNAME_COMPONENT_SAMPLE()).append(remoteSuffix).append(Encryptor::getNAME_COMPONENT_TO());
    secUserName.append(selfPrefix).append(Encryptor::getNAME_COMPONENT_SAMPLE()).append(selfSuffix);
    
    impl_ = new Impl (remotePrefix,remoteSuffix, selfPrefix, selfSuffix,&secUserMainFace_, &keyChain_, groupName, secUserName, getNO_LINK(), getNO_LINK());
...
void
SecUser::registAndNegotiateContentKey(Name remotePrefix,Name remoteSuffix, Name selfPrefix, Name selfSuffix)
{
        ...
        //Name consumerName = Name("/Prefix/SAMPLE/Content");
         //Name registerName("/Prefix/REGISTER/content");       
        //string databaseFilePath="./consumer.db";
   
        //ptr_lib::shared_ptr<ConsumerDb> testDb(new Sqlite3ConsumerDb(databaseFilePath));

    
        //register secUser,will expreess register interest to the producer
        this->registUser(remotePrefix,remoteSuffix, selfPrefix, selfSuffix);
    
        this->addDecryptionKeyofCkey(remotePrefix,remoteSuffix, selfPrefix, selfSuffix);
    
        this->askSecControlerBeReady(remotePrefix,remoteSuffix, selfPrefix, selfSuffix);
        
        //will expreess secure content interest to the producer 
        this->getContentKeyFromSecControler(remotePrefix,remoteSuffix, selfPrefix, selfSuffix);
    
        cout<<"registAndNegotiateContentKey excuted!"<<endl;
...
void 
SecUser::registUser(Name &remotePrefix,Name &remoteSuffix, Name &selfPrefix, Name &selfSuffix)
{
    //Name registerName("/Prefix/REGISTER/content");
    
    //ptr_lib::shared_ptr<ConsumerDb> testDb(new Sqlite3ConsumerDb(databaseFilePath));

    Name registerName(remotePrefix);
    registerName.append(Encryptor::getNAME_COMPONENT_REGISTER()).append(remoteSuffix)\
        .append(Encryptor::getNAME_COMPONENT_FOR()).append(selfPrefix).append(Encryptor::getNAME_COMPONENT_SAMPLE()).append(selfSuffix);
    ...
    registerInterest.setMustBeFresh(true);
...
{
    cout << "Time out for interest " << interest->getName().toUri() << endl;     
    askReadyFace_.expressInterest(*interest, bind(&SecUser::onReadyData, this,_1,_2 ), bind(&SecUser::onReadyTimeout, this, _1));
}

void 
SecUser::askSecControlerBeReady(Name &remotePrefix,Name &remoteSuffix, Name &selfPrefix, Name &selfSuffix)
{

    //Name askReady("/Prefix/READY/SecControler/TO/Prefix/SAMPLE/SecUser");
    Name askReady(remotePrefix);
    askReady.append(Encryptor::getNAME_COMPONENT_READY()).append(remoteSuffix).append(Encryptor::getNAME_COMPONENT_TO())\
        .append(selfPrefix).append(Encryptor::getNAME_COMPONENT_SAMPLE()).append(selfSuffix);
    
    Interest requestConsumeInterest(askReady);
    requestConsumeInterest.setMustBeFresh(true);
    
    askReadyFace_.setCommandSigningInfo(keyChain_, keyChain_.getDefaultCertificateName());
    askReadyFace_.expressInterest(requestConsumeInterest, bind(&SecUser::onReadyData, this,_1,_2 ), bind(&SecUser::onReadyTimeout, this, _1));

    while(!beReadytoConsume_)
    {
        askReadyFace_.processEvents();
    }
}
...
void
SecUser::addDecryptionKeyofCkey(Name &remotePrefix,Name &remoteSuffix, Name &selfPrefix, Name &selfSuffix)
{
    MillisecondsSince1970 timeSlot= fromIsoString("20150101T100000");
    //Name dKeyName = Name("/Prefix/SAMPLE/Content/D-KEY/20150101T100000/20150101T120000");

    Name dKeyName(remotePrefix);
    dKeyName.append(Encryptor::getNAME_COMPONENT_SAMPLE()).append(remoteSuffix).append(Encryptor::getNAME_COMPONENT_TO()).append(selfPrefix);
    dKeyName.append(Encryptor::getNAME_COMPONENT_SAMPLE()).append(selfSuffix).append(Encryptor::getNAME_COMPONENT_D_KEY()).append("20150101T100000").append("20150101T120000");
...
void 
SecUser::getContentKeyFromSecControler(Name &remotePrefix,Name &remoteSuffix,Name &selfPrefix, Name &selfSuffix)
{
    int finalCount = 0;
    //Name contentName = Name("/Prefix/SAMPLE/Content/FOR/Prefix/SAMPLE/selfSuffix");

    Name contentName(remotePrefix);
    contentName.append(Encryptor::getNAME_COMPONENT_SAMPLE()).append(remoteSuffix);
    contentName.append(Encryptor::getNAME_COMPONENT_TO()).append(selfPrefix).append(Encryptor::getNAME_COMPONENT_SAMPLE()).append(selfSuffix);
    ...
    cout<< "getContentKeyFromSecControler face address:" << impl_->face_ << std::endl;
    ...
    cout<<"getContentKeyFromSecControler excuted!"<<contentName<<endl;
...
SecUser::Impl::Impl
  (Face* face, KeyChain* keyChain, const Name& groupName,
   const Name& secUserName, const ptr_lib::shared_ptr<ConsumerDb>& database,
   const Link& cKeyLink, const Link& dKeyLink)
: face_(face),
  keyChain_(keyChain),
  groupName_(groupName),
  secUserName_(secUserName),
  database_(database),
...
//Impl without database
SecUser::Impl::Impl
  (Name &remotePrefix,Name &remoteSuffix, Name &selfPrefix, Name &selfSuffix,Face* face, KeyChain* keyChain, const Name& groupName,
   const Name& secUserName,const Link& cKeyLink, const Link& dKeyLink)
    : secUserRemotePrefix_(remotePrefix),
    secUserRemoteSuffix_(remoteSuffix),
    secUserSelfPrefix_(remoteSuffix),
    secUserSelfSuffix_(selfSuffix),
    face_(face),
    keyChain_(keyChain),
    groupName_(groupName),
    secUserName_(secUserName),
  ...
  public:
    ...
    onContentVerified(const ptr_lib::shared_ptr<Data>& validData)
    {
      // Save this for calling onConsumeComplete.
        ...
        cout<<"SecUser::Impl::consume onContentVerified "<<contentData_->getName().toUri()<<endl;
    ...
    onContentPlainText(const Blob& plainText)
    {
	  ...
	  cout<<"SecUser::Impl::consume onContentPlainText"<<endl;
    }
...
{
  if (!(secUserName_.match(keyName)))
    throw runtime_error
      ("SecUser addDecryptionKey: The secUser name must be a prefix of the key name");
    ...
    cout<<"SecUser::Impl::consume addDecryptionKey in database"<<endl;
   ...
   const OnPlainText& onPlainText, const EncryptError::OnError& onError)
{
  ...
  try {
    ...
    return;
	cout<<"SecUser::Impl::consume decrypt"<<endl;
  }
   ...
   const OnPlainText& onPlainText, const EncryptError::OnError& onError)
{
    ...
    for(int i =0;i<keyBits.size();i++)
  {
      if (0==i)
    {
        printf("SecUser::Impl:: decryptEncryptedContent Decryption key\n");
    }
  ...
  if (encryptedContent.getAlgorithmType() == ndn_EncryptAlgorithmType_AesCbc) {
	  ...
	  for(int i =0;i<content.size();i++)
	{
		if (0==i)
	  {
		  printf("SecUser::Impl:: AesAlgorithm::decrypt\n");
	  }
   ...
   const EncryptError::OnError& onError)
{
  ...
  cout<<"SecUser::Impl::consume decryptContent getKeyLocator"<<cKeyName<<endl;
   ...
   const EncryptError::OnError& onError)
{
  ...
  cout<<"SecUser::Impl::consume decryptCKey"<<endl;
  // Make this a shared_ptr so we can pass it in callbacks.
  ...
  cout << "SecUser::Impl::decryptCKey dKeyName:" <<dKeyName<<endl;
  ...
  if (dKeyMap_.find(dKeyName) != dKeyMap_.end())
    ...
    interestName.append(Encryptor::getNAME_COMPONENT_FOR()).append(secUserName_);
    ptr_lib::shared_ptr<const Interest> interest(new Interest(interestName));
   ...
   const EncryptError::OnError& onError)
{
  ...
  cout<<"SecUser::Impl::consume decryptDKey"<<endl;
  ...
  if (consumerKeyBlob.size() == 0) {
    try {
      onError(EncryptError::ErrorCode::NoDecryptKey,
        "The desired SecUser decryption key in not in the database");
    } catch (const std::exception& ex) {
      _LOG_ERROR("Error in onError: " << ex.what());
    } catch (...) {
      _LOG_ERROR("Error in onError.");
   ...
   const EncryptError::OnError& onError)
{
  ...
  cout<<"SecUser::Impl::consume sendInterest -->"<<interest->getName().toUri()<<endl;
  ...
  public:
       ...
       const ptr_lib::shared_ptr<Data>& contentData)
    {
	  ...
	  cout<<"SecUser::Impl::consume sendInterest-> onData->"<<contentInterest->getName()<<endl;
       ...
       const ptr_lib::shared_ptr<NetworkNack>& networkNack)
    {
      ...
      try {
        ...
        _LOG_ERROR("Error in onError.");
         ...
         cout<<"SecUser::Impl::consume sendInterest onNetworkNack:"<< interest->getName() << endl;
    }
    ...
    onTimeout(const ptr_lib::shared_ptr<const Interest>& interest)
    {
	  ...
	  cout<<"SecUser::Impl::consume sendInterest onTimeout:"<< interest->getName()<<endl;
    }
   ...
   const EncryptError::OnError& onError)
{
  ...
  cout<<"SecUser::Impl::consume onValidationFailed"<<endl;
}
