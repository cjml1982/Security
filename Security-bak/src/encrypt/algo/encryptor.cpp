   ...
   const EncryptParams& params)
{
    ...
    for(int i =0;i<initialVector.size();i++)
  {
      if (0==i)
  	{
  	    printf("Encryptor.cpp encryptSymmetric    initialVector\n");
  	}
      printf("0x%x,",initialVector.buf()[i]);	  
      if (i%8==7)
	  	printf("\n");
  }

  printf("initialVector.size()=%d\n",initialVector.size());
  ...
  if (algorithmType == ndn_EncryptAlgorithmType_AesCbc ||
	  ...
	  for(int i =0;i<encryptedPayload.size();i++)
	{
		if (0==i)
	  {
		  printf("Encryptor.cpp encryptSymmetric	encryptedPayload\n");
	  }
		printf("0x%x,",encryptedPayload.buf()[i]); 	
		if (i%8==7)
		  printf("\n");
	}
   ...
   const EncryptParams& params)
{
  ...
  if (algorithmType == ndn_EncryptAlgorithmType_RsaPkcs ||
    	  ...
    	  for(int i =0;i<encryptedPayload.size();i++)
	{
		if (0==i)
	  {
		  printf("Encryptor.cpp encryptAsymmetric	encryptedPayload\n");
	  }
		printf("0x%x,",encryptedPayload.buf()[i]); 	
		if (i%8==7)
		  printf("\n");
