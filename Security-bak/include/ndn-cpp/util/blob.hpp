...
public:
   ...
   * @param blobStruct The C pki_Blob struct to receive the pointer. However, if
   ...
   */
  std::string
  toHex() const { return (*this) ? pki::toHex(**this) : std::string(); }
   ...
   * @param blobStruct The C pki_Blob struct to receive the pointer.
   */
  void
