...
*/
    SecControler(Name &remotePrefix,Name &remoteSuffix, Name &selfPrefix, Name &selfSuffix);
    ...
    void 
    getEkeyFromSecUser(Name remotePrefix,Name remoteSuffix);

    void
    processSecUserEventGetContentKey(Name &remotePrefix,Name &remoteSuffix, Name &selfPrefix, Name &selfSuffix);    
    ...
    void 
    processGetContentKey(Name &remotePrefix,Name &remoteSuffix, Name &selfPrefix, Name &selfSuffix);
    ...
    void
    processReadyRequest(Name remotePrefix,Name remoteSuffix,Name selfPrefix, Name selfSuffix);

    void
    getContentKey(Name &keyName,Blob *contentKeyBits)
    {
        impl_->getContentKey(keyName,contentKeyBits);
    }

    bool
    hasContentKey(Name &keyName)
    {
        return impl_->hasContentKey(keyName);
    }    
...
private:
  ...
  public:
    ...
    hasContentKey(Name &keyName)
    {
        if (cKeySecControlerMap_.find(keyName)!=cKeySecControlerMap_.end())
        {
            cout<<"hasContentKey = ture:"<<keyName<<endl;
            return true;
        }
        cout<<"hasContentKey = false: keyName "<<keyName<<endl;
        return false;
    }
    ...
    getContentKey(Name &keyName,Blob *contentKeyBits)
    {
        if (cKeySecControlerMap_.find(keyName)!=cKeySecControlerMap_.end())
        {     
            *contentKeyBits = cKeySecControlerMap_[keyName];
        }           
    }
  ...
  private:
    ...
    Face* face_;
    Name secControlerRemotePrefix_;
    Name secControlerRemoteSuffix_;
    Name secControlerSelfPrefix_;
    Name secControlerSelfSuffix_;
    ...
    std::map<Name, Blob> cKeySecControlerMap_;
