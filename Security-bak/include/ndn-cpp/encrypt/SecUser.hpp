...
#ifndef NDN_SECUSER_HPP
#define NDN_SECUSER_HPP
...
#include "SecCommon.hpp"

/*
#modify by Marty, May 16th ,2017
#consumerAdapter.hpp for the  data content security
#modify history: 
...
public:
  ...
  Consumer
    (Face* face, KeyChain* keyChain, const Name& groupName,
     ...
     const Link& cKeyLink = getNO_LINK(), const Link& dKeyLink = getNO_LINK())
  : impl_(new Impl
      (face, keyChain, groupName, consumerName, cKeyLink, dKeyLink))
  {
  }
*/
    SecUser(Name &remotePrefix,Name &remoteSuffix, Name &selfPrefix, Name &selfSuffix);
  ~SecUser();

    void
    registAndNegotiateContentKey(Name remotePrefix,Name remoteSuffix, Name selfPrefix, Name selfSuffix);
  ...
  void
  addDecryptionKeyofCkey(const Name& keyName, const Blob& keyBlob)
  {
    impl_->addDecryptionKeyofCkey(keyName, keyBlob);
  }

 void
 onRegistedData(const ptr_lib::shared_ptr<const Interest>& interest, const ptr_lib::shared_ptr<Data>& data);

 void 
onRegistedTimeout(const ptr_lib::shared_ptr<const Interest>& interest);


 void 
 registUser(Name &remotePrefix,Name &remoteSuffix, Name &selfPrefix, Name &selfSuffix );
 void
 onReadyData(const ptr_lib::shared_ptr<const Interest>& interest, const ptr_lib::shared_ptr<Data>& data);
 void 
 onReadyTimeout(const ptr_lib::shared_ptr<const Interest>& interest);
 
 void 
 askSecControlerBeReady(Name &remotePrefix,Name &remoteSuffix, Name &selfPrefix, Name &selfSuffix);

 void addFixedContentKey(Name& cKeyName);

 void addDecryptionKeyofCkey(Name &remotePrefix,Name &remoteSuffix, Name &selfPrefix, Name &selfSuffix);

 void setRegisted();

 void getContentKeyFromSecControler(Name &remotePrefix,Name &remoteSuffix,Name &selfPrefix, Name &selfSuffix);
 
 public:

    Face registerFace_;
    Face askReadyFace_;
    Face secUserMainFace_;
    KeyChain keyChain_;
    bool registered_;
    bool beReadytoConsume_;
...
private:
  ...
  public:
    ...
    Impl
      (Name &remotePrefix,Name &remoteSuffix, Name &selfPrefix, Name &selfSuffix,Face* face, KeyChain* keyChain, const Name& groupName,const Name& consumerName,const Link& cKeyLink, const Link& dKeyLink);
    ...
    void
    addContentKey(const Name& keyName, const Blob& keyBlob);
  ...
  private:
    ...
    //for baar thers is no disk to save the database,so this database reserved 
public:
    ...
    Name secUserRemotePrefix_;
    Name secUserRemoteSuffix_;
    Name secUserSelfPrefix_;
    Name secUserSelfSuffix_;
