...
public:
  ...
  typedef pki::func_lib::function<void
    (ChannelDiscovery& channelDiscovery,
     const pki::ptr_lib::shared_ptr<pki::Name>& userPrefix)> OnReceivedChannelList;
   ...
   * /pki/edu/ucla/remap/peter/flume/discovery .
   ...
   * interests and sync updates. For example, /pki/broadcast/flume/discovery .
   ...
   */
  ChannelDiscovery
    (const pki::Name& applicationDataPrefix,
     const std::string& channelListFilePath,
     const pki::Name& applicationBroadcastPrefix, pki::Face& face, 
     pki::KeyChain& keyChain, const pki::Name& certificateName,
     pki::Milliseconds syncLifetime, 
   ...
   */
  pki::ptr_lib::shared_ptr<std::vector<pki::Name>>
   ...
   * does not exist.
   */
  pki::ptr_lib::shared_ptr<std::vector<std::string>>
  getChannelList(const pki::Name& userPrefix)
  {
...
private:
   ...
   * use in callbacks.
   */
  class Impl : public pki::ptr_lib::enable_shared_from_this<Impl> {
  public:
    /**
     ...
     */
    Impl
      (ChannelDiscovery& parent, const pki::Name& applicationDataPrefix,
       const std::string& channelListFilePath, pki::Face& face,
       pki::KeyChain& keyChain, const pki::Name& certificateName,
       pki::Milliseconds syncLifetime,
       const OnReceivedChannelList& onReceivedChannelList, const OnError& onError);
     ...
     */
    void
    initialize(const pki::Name& applicationBroadcastPrefix);
    ...
    onRegisterApplicationPrefixSuccess
      (const pki::ptr_lib::shared_ptr<const pki::Name>& prefix, 
       uint64_t registeredPrefixId, const pki::Name& applicationBroadcastPrefix,
    ...
    void
    onReceivedSyncState
      (const std::vector<pki::ChronoSync2013::SyncState>& syncStates,
       bool isRecovery);
    ...
    void
    onData
      (const pki::ptr_lib::shared_ptr<const pki::Interest>& interest,
       const pki::ptr_lib::shared_ptr<pki::Data>& data);
    ...
    onApplicationInterest
      (const pki::ptr_lib::shared_ptr<const pki::Name>& prefix,
       const pki::ptr_lib::shared_ptr<const pki::Interest>& interest,
       pki::Face& face, uint64_t interestFilterId,
       const pki::ptr_lib::shared_ptr<const pki::InterestFilter>& filter);
     ...
     */
    void
    onRegisterFailed(const pki::ptr_lib::shared_ptr<const pki::Name>& prefix);
     ...
     */
    void
    onTimeout(const pki::ptr_lib::shared_ptr<const pki::Interest>& interest);
     ...
     */
    pki::ptr_lib::shared_ptr<pki::Data>
    ...
    static void
    parseChannelList
      (const pki::Blob& blob, std::vector<std::string>& channelList);
     ...
     */
    static void
    writeBase64Blob(const pki::Blob& blob, const std::string& filePath);

  private:
    ChannelDiscovery& parent_;
    pki::Name applicationDataPrefix_;
    int sessionNo_;
    std::string channelListFilePath_;
    pki::Face& face_;
    pki::KeyChain& keyChain_;
    pki::Name certificateName_;
    pki::Milliseconds syncLifetime_;
    OnReceivedChannelList onReceivedChannelList_;
    OnError onError_;
    pki::ptr_lib::shared_ptr<pki::ChronoSync2013> sync_;
    ...
    pki::ptr_lib::shared_ptr<pki::Data> channelListData_;
    bool enabled_;
  };

  pki::ptr_lib::shared_ptr<Impl> impl_;
};
