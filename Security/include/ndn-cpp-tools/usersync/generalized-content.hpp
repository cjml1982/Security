 ...
 */
class GeneralizedContent : public pki::ptr_lib::enable_shared_from_this<GeneralizedContent> {
public:
  enum ErrorCode {
  ...
  typedef pki::func_lib::function<void
    (const pki::ptr_lib::shared_ptr<ContentMetaInfo>& metaInfo,
     const pki::Blob& content)> OnComplete;
  ...
  static void
  publish
    (pki::MemoryContentCache& contentCache, const pki::Name& prefix,
     pki::Milliseconds freshnessPeriod, pki::KeyChain* signingKeyChain,
     const pki::Name& signingCertificateName, const ContentMetaInfo& metaInfo,
     const pki::Blob& content, size_t contentSegmentSize);
  ...
  static void
  fetch
    (pki::Face& face, const pki::Name& prefix, pki::KeyChain* validatorKeyChain,
     const OnComplete& onComplete, const OnError& onError,
     pki::Milliseconds interestLifetimeMilliseconds = 4000.0);
...
private:
  ...
  GeneralizedContent
    (pki::Face& face, const pki::Name& prefix, pki::KeyChain* validatorKeyChain,
     const OnComplete& onComplete, const OnError& onError,
     pki::Milliseconds interestLifetimeMilliseconds)
  ...
  void
  onMetaInfoReceived
    (const pki::ptr_lib::shared_ptr<const pki::Interest>& originalInterest,
     const pki::ptr_lib::shared_ptr<pki::Data>& data);

  void
  onMetaInfoTimeout(const pki::ptr_lib::shared_ptr<const pki::Interest>& interest);

  void
  onContentReceived(const pki::Blob& content);

  void
  onSegmentFetcherError
    (pki::SegmentFetcher::ErrorCode errorCode, const std::string& message);

  pki::Face& face_;
  pki::Name prefix_;
  pki::KeyChain* validatorKeyChain_;
  OnComplete onComplete_;
  OnError onError_;
  pki::Milliseconds interestLifetimeMilliseconds_;
  pki::ptr_lib::shared_ptr<ContentMetaInfo> metaInfo_;
};
