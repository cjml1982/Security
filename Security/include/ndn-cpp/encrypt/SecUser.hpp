...
*/
  ...
  ~SecUser();
    ...
    void 
    Init(const Name& groupName,const Name& consumerName);
...
onRegistedTimeout(const ptr_lib::shared_ptr<const Interest>& interest);
 ...
 void 
 registerConsumer(Interest & registerInterest );
 void
 ...
 void 
 requestConsumeReady(Name &requestConsume);

 void addFixedContentKey(Name& cKeyName);

 void addDecryptionKeyofCkey(Name& cKeyName);

 void setRegisted();

 void consumeSecureContent(Name& contentName);
...
private:
  ...
  public:
    ...
    Impl
      (Face* face, KeyChain* keyChain, const Name& groupName,const Name& consumerName,const Link& cKeyLink, const Link& dKeyLink);
